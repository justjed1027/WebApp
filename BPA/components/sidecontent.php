...
function renderSideContent($currentPage = '', $options = [])
{
    ...
    if (isset($options['hide']) && is_array($options['hide'])) {
        ...
        if (in_array('notifications', $hide)) {
            $showNotifications = false;
        }
        if (in_array('upcomingevents', $hide) || in_array('upcoming_events', $hide)) {
            $showUpcomingEvents = false;
        }
        if (in_array('recentdms', $hide) || in_array('recent_dms', $hide) || in_array('messages', $hide)) {
            $showRecentDMs = false;
        }
        if (in_array('suggestedcollaborators', $hide) || in_array('suggested_collaborators', $hide) || in_array('collaborators', $hide)) {
            $showSuggestedCollaborators = false;
        }
        // Trending topics kept for backward compatibility but no longer shown by default
        if (in_array('trendingtopics', $hide) || in_array('trending_topics', $hide) || in_array('topics', $hide)) {
            $showTrendingTopics = false;
        }
    }
