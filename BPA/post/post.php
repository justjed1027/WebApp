...
<body class="has-side-content">
  ...
  <!-- Sidebar Navigation (copied from courses page for working hover expand) -->
            ...
            <?php else: ?>
              <?php foreach ($posts as $post): ?>
                <div class="post">
                  <div class="post-header">
                    ...
                    <div style="display:flex;align-items:center;gap:12px;flex:1;">
                      ...
                      <div style="display:flex;flex-direction:column;">
                        <a href="../profile/profile.php?user_id=<?php echo intval($post['user_id']); ?>" class="post-author-link"><?php echo htmlspecialchars($displayName); ?></a>
                        <div class="post-timestamp"><?php echo isset($post['created_at']) ? htmlspecialchars(timeAgo($post['created_at'])) : 'just now'; ?></div>
                      </div>
                    </div>
                  ...
                  <div class="post-content" style="line-height:1.5;">
                  ...
                  <div class="post-delete-confirmation" data-post-id="<?php echo intval($post['post_id']); ?>" style="display:none;background:#fff3cd;border:1px solid #ffecb5;border-radius:4px;padding:8px 10px;margin-top:8px;text-align:right;max-width:fit-content;margin-left:auto;">
                    <div style="font-size:0.8rem;margin-bottom:6px;text-align:left;color:#856404;">Delete this post?</div>
    ...
    <div id="postDetailModal" class="modal-backdrop" role="dialog" aria-hidden="true">
      <div class="modal-box" id="postDetailBox">
        ...
        <div style="display:flex;gap:12px;align-items:center;margin-bottom:12px;">
          ...
          <div>
            <div id="postDetailUser" style="font-weight:700;color:var(--text-primary)"></div>
            <div id="postDetailTime" class="post-timestamp" style="font-size:0.85rem;"></div>
          </div>
        </div>
          ...
          .then(data => {
            if (data.success) {
              if (data.comments.length === 0) {
                commentsList.innerHTML = '<div style="color:var(--text-secondary);font-size:0.85rem;text-align:center;padding:12px;">No comments yet. Be the first to comment!</div>';
              } else {
                let html = '';
                    ...
                    <div class="comment-item" style="display:flex;gap:10px;margin-bottom:14px;padding:10px;background:var(--background-hover);border-radius:6px;">
                      ...
                      <div style="flex:1;">
                        <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                          <a href="../profile/profile.php?user_id=${comment.user_id}" class="comment-author-link">${escapeHtml(comment.username)}</a>
                          <span class="post-timestamp" style="font-size:0.8rem;">${escapeHtml(comment.time_ago)}</span>
                        </div>
